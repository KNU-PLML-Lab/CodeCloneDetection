...
MODEL_CLASSES = {'codet5': (T5Config, T5ForConditionalGeneration, RobertaTokenizer),
                 ...
                 'codet5p': (T5Config, T5ForConditionalGeneration, RobertaTokenizer)
}
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                logger.info("  "+"*"*20)   
                writeresult = 'recall: ' + str(round(temp_best_recall, 4)) + ' precision:' + str(round(temp_best_precision, 4)) + \
                    ' F1:'+str(round(temp_best_f1, 4))+' tau:' + str(tau) + \
                    ' threshold:' + \
                    str(round(temp_best_threshold, 1)) + \
                    ' epoch:'+str(epoch)+'\n'
                f = open('result_codet5p.txt','a+')
                f.write(writeresult)
                f.close()
